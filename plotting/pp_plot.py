...
def main(args):
    ...
    result_files = {}
    for filename in args.result_dir.glob("*.hdf5"):
        match = re.search(r'injection_(\d+)', str(filename))
        if match:
            inj_id = match.group(1)
            result_files[inj_id] = filename
        else:
            print(f"Could not find injection ID in {filename}")
